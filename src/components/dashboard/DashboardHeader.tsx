 ...
 * text-slate-400 (7,6:1), botao com alvo de 44px no mobile e foco em anel
 * laranja. O texto de atualizacao vive numa regiao aria-live: quem usa leitor
    ...
    <header className="sticky top-0 z-50 border-b border-white/[0.06] bg-slate-950/85 backdrop-blur-xl">
      <div className="flex items-center justify-between gap-3 px-3 py-2 sm:px-4">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          <div
            aria-hidden="true"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-orange-500 to-red-600 shadow-lg shadow-orange-500/25"
          >
            <Flame className="h-4 w-4 text-white" />
          </div>
          <div className="flex min-w-0 items-center gap-2 sm:gap-3">
            <h1 className="text-[22px] font-bold leading-none tracking-tight text-white sm:text-[26px]">
              HypeFC
            </h1>
            {hasLiveMatches && (
              <span className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-emerald-500/15 px-2 py-1 text-[11px] font-semibold uppercase tracking-wider text-emerald-300">
                <span aria-hidden="true" className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" />
                ao vivo
              </span>
            )}
            <p className="hidden min-w-0 truncate text-xs text-slate-400 md:block">
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          <div aria-live="polite" className="hidden items-center gap-2 text-xs text-slate-400 sm:flex">
            {hora ? (
              <>
                <Wifi aria-hidden="true" className="h-3.5 w-3.5 text-emerald-400" />
                <span title={lastUpdated}>Atualizado às {hora}</span>
              </>
            ) : (
              <>
                <WifiOff aria-hidden="true" className="h-3.5 w-3.5 text-slate-400" />
                <span>Sem dados</span>
              </>
            )}
          </div>
          <button
            ...
            aria-busy={isLoading}
            className="group flex h-11 w-11 items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/5 text-[13px] font-medium text-slate-300 transition-colors hover:border-white/20 hover:bg-white/10 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-400/60 active:bg-white/[0.14] disabled:cursor-not-allowed disabled:opacity-50 sm:h-auto sm:min-h-[36px] sm:w-auto sm:px-3"
          >
            <RefreshCw
              aria-hidden="true"
              className={`h-4 w-4 transition-transform group-hover:text-orange-400 ${isLoading ? 'animate-spin' : ''}`}
            />
