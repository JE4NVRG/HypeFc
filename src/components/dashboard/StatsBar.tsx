 ...
 * absoluto).
 *
 * Cor: os icones sao neutros (ink-3 em caixa neutra) porque icone colorido nao
 * significa nada — e cores diferentes lado a lado viram ruido. Verde fica
...
function StatCard({
  ...
  return (
    <div className="flex min-h-[56px] min-w-[104px] shrink-0 items-center gap-2 rounded-xl border border-ink/[0.06] bg-ink/[0.02] px-2 py-2 sm:min-h-[60px] sm:gap-3 sm:px-3 lg:min-w-0">
      <div
        aria-hidden="true"
        className={`h-8 w-8 shrink-0 items-center justify-center rounded-lg ${
          live ? 'flex bg-verde/10 text-verde-2' : 'hidden bg-ink/[0.06] text-ink-3 sm:flex'
        }`}
      >
        {icon}
      </div>
      <div className="min-w-0">
        <p className="text-[20px] font-bold leading-none tracking-tight text-ink tabular-nums sm:text-[28px]">{value}</p>
        <p className="mt-1 text-[11px] font-medium uppercase leading-none tracking-wider text-ink-3">{label}</p>
      </div>
    </div>
  )
}
    ...
    return (
      <section aria-labelledby={TITULO_ID}>
        ...
        <div aria-hidden="true" className="grid grid-cols-3 gap-2 lg:grid-cols-5">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-[56px] animate-pulse rounded-xl bg-ink/5 sm:h-[60px]" />
          ))}
        </div>
          ...
          <StatCard
            icon={
              <span className="relative flex h-4 w-4 items-center justify-center">
                <span className="absolute h-3 w-3 animate-ping rounded-full bg-verde/40" />
                <span className="relative h-2 w-2 rounded-full bg-verde" />
              </span>
            }
            label="Ao vivo"
