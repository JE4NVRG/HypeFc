...
function FixtureRow({ fixture }: { fixture: TeamFixture }) {
  return (
    <div className="flex items-center gap-2 text-[11px]">
      <span className="w-8 shrink-0 text-[11px] uppercase tracking-wide text-ink-3">
      ...
      {fixture.competition ? (
        <span className="max-w-[88px] shrink-0 truncate text-[11px] text-ink-3" title={fixture.competition}>
          {fixture.competition}
        </span>
      ) : null}
      <span className="w-10 shrink-0 text-right font-mono text-[11px] text-ink-3" title={fixtureDayTitle(fixture.date)}>
  ...
  return (
    <section className="rounded-xl border border-paper-3 bg-paper-2/40 p-3">
      <div className="mb-2 flex items-baseline justify-between gap-2">
        <h3 className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-wide text-ink-3">
          <Clock aria-hidden="true" className="h-3.5 w-3.5 shrink-0" />
          Próximos jogos
        </h3>
        {team ? (
          <span className="truncate text-[11px] text-ink-3" title={team}>
            {team}
          </span>
        ) : null}
      </div>
      ...
      {loading ? (
        <div className="space-y-2">
          {[...Array(3)].map((_, index) => (
            <div key={index} className="h-4 w-full animate-pulse rounded bg-ink/5" />
          ))}
        </div>
      ) : list.length ? (
        <div className="space-y-2">
        ...
        <p className="text-[11px] leading-snug text-ink-3">Próximos jogos ainda não publicados para este time.</p>
      )}
    </section>
  )
}
